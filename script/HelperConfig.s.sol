...
contract HelperConfig is Script {
    ...
    constructor() {
        if (block.chainid == 11155111) { // Sepolia Ethereum Chain ID 
            activeNetworkConfig = getSepoliaEthConfig();
        } else if (block.chainid == 300) { // zkSync sepolia Chain ID
            activeNetworkConfig = getZkSyncSepoliaEthConfig();
        } else if (block.chainid == 1) { // Mainnet Ethereum Chain ID
            activeNetworkConfig = getMainnetEthConfig();
        } else { // local anvil chain (local blockchain)
